...

{
	...
	*/	
    ...
    public function serverUrlAction(){
        echo json_encode(array("error_code"=>'0', 'response_string'=>'success.', 'serverurl'=>'http://soufflelive.runapp.me/api'));
        exit;
        }
    ...
    public function loginAction(){ 
		...
		try{
			...
			if($alldata['App_Access_Status'] == 2){
                        ...
                        if($alldata['TotalVillage'] == 0){
                            $error_code = 4;
                            throw new Exception('No village mapping,please contact admin');
                        }
			...
			$notificationData['noti_date'] = date('Y-m-d H:i:s');
                        ...
                        $GetLANDSHOWINGDETAILBYFELOGINID= $user->GetLANDSHOWINGDETAILBYFELOGINID($LoginID);
                        $GetLANDPREPRATIONDETAILBYFELOGINID = $user->GetLANDPREPRATIONDETAILBYFELOGINID($LoginID);
                        $GetLANDIRRIGATIONDETAILBYFELOGINID = $user->GetLANDIRRIGATIONDETAILBYFELOGINID($LoginID);
                        $GetLANDFERTILIZERDETAILBYFELOGINID = $user->GetLANDFERTILIZERDETAILBYFELOGINID($LoginID);
                        $GetLANDVISITDETAILBYFELOGINID = $user->GetLANDVISITDETAILBYFELOGINID($LoginID);                      
                        $GetCROPPROTECTIONDETAILBYFELOGINID = $user->GetCROPPROTECTIONDETAILBYFELOGINID($LoginID);                        
                        $GetLANDINFORMATIONDETAILBYFELOGINID = $user->GetLANDINFORMATIONDETAILBYFELOGINID($LoginID);                        
                        $GetLANDYIELDDETAILBYFELOGINID = $user->GetLANDYIELDDETAILBYFELOGINID($LoginID);                        
                        //------------------------------------------------
                        $GetBankDetail = $user->GetBankList();
                        ...
                        $getAllSeedOrdersTotal = $seedMaster->getAllSeedOrdersTotal($LoginID);
                        $seedOrderTotal=$getAllSeedOrdersTotal['val'];
                        
                        $getAllChemicalOrdersTotal = $seedMaster->getAllChemicalOrdersTotal($LoginID);
                        $ChemicalOrderTotal=$getAllChemicalOrdersTotal['val'];
                    
                        // Dc list
                        $dc_list = $user->getAllUserDCByRegionID($alldata['userId']);
                        
                        $query = "select * from logi_tcwofseed  where status=0";
                        $lotDetails = $db->fetchAll($query);
		...
		if($succes == TRUE ){
			...
			echo json_encode(array("error_code"=>'0', 'response_string'=>'login success.','LastCrop'=>$LastCrop,'RabiCrop'=>$RabiCrop,
                            ...
                            'UserData'=>$StaffData,'CircleData'=>$circleData,'DistrictData'=>$districtData,'TehsilData'=>$tehsilData,
                            'RegionData'=>$regionData,'VillData'=>$villData,'AllUnits'=>$units,'SeedType'=>$seedType,'SeedSubType'=>$seedSubType,
                            'SeedRowType'=>$seedRowType,'SeedList'=>$seedList,'DcList'=>$dc_list,'seedOrderTotal'=>$seedOrderTotal,
                            'ChemicalOrderTotal'=>$ChemicalOrderTotal,'Lot_List'=>$lotDetails ,'Bank_detail'=>$GetBankDetail ,'Vat'=>$GetVatData ,
                            'FE_Showing_detail'=>$GetLANDSHOWINGDETAILBYFELOGINID,'FE_prepration_detail'=>$GetLANDPREPRATIONDETAILBYFELOGINID,
                            'FE_irigation_detail'=>$GetLANDIRRIGATIONDETAILBYFELOGINID ,'FE_fertilizer_detail'=>$GetLANDFERTILIZERDETAILBYFELOGINID ,
                            'FE_visit_detail'=>$GetLANDVISITDETAILBYFELOGINID ,'FE_land_crop_protection_detail'=>$GetCROPPROTECTIONDETAILBYFELOGINID ,
                            'FE_Land_information_detail'=>$GetLANDINFORMATIONDETAILBYFELOGINID, 'FE_yield_detail'=>$GetLANDYIELDDETAILBYFELOGINID ));
			exit;
		}

		else{
			echo json_encode(array("error_code"=>$error_code, 'response_string'=>$error, 'params' =>$params ));
			exit;
		}
...

    {
		...
		// Genaral Information of the user login details
		 ...
		 //$wayofshowing	= isset($params['wayofshowing'])?$params['wayofshowing']:"";
                 ...
                 $NoOfTractor	= isset($params['NoOfTractor'])?$params['NoOfTractor']:"";
                 $NoOfHarrrow	= isset($params['NoOfHarrrow'])?$params['NoOfHarrrow']:"";
                 $NoOfRotavator	= isset($params['NoOfRotavator'])?$params['NoOfRotavator']:"";
                 $NoOfThresher	= isset($params['NoOfThresher'])?$params['NoOfThresher']:"";
...

		{
			...
			// Add land Information of farmer
                        ...
                        $insertFarmerDataShowing['NoOfTractor'] = $NoOfTractor;
                        $insertFarmerDataShowing['NoOfHarrrow'] = $NoOfHarrrow;
                        $insertFarmerDataShowing['NoOfRotavator'] = $NoOfRotavator;
                        $insertFarmerDataShowing['NoOfThresher'] = $NoOfThresher;
	...
	*/
   	...
   	public function seedOrderAction(){
		...
		// Genaral Information of the user login details
                 ...
                 $lot_id = isset($params['lot_id'])?$params['lot_id']:"";
                // $tcw = isset($params['tcw'])?$params['tcw']:"";
		...
		try{
	        ...
	        if($alldata['App_Access_Status'] == 2){
                ...
                $farmerMaster = new Application_Model_Farmermasters();
              //  $allTcwSeedData = $farmerMaster->getTcwDetails();
                ...
                $insertFarmerData['lot_id'] = $lot_id;
                ...
                //$insertFarmerData['PaidAmount'] = $payment_recevied; 
               ...
               // $insertFarmerData['tcw'] = $tcw;
                ...
                if(isset($_FILES['photograph2']['tmp_name']) AND !empty($_FILES['photograph2']['tmp_name'])){
                        ...
                        $imageName1 = time().$_FILES['photograph2']['name']; 
                        $uploads1 = 'uploads/seed_orders/';
                        if(!file_exists($uploads1)){
                                mkdir($uploads1);	
                        }
                        $pathComplete1 = $uploads1.$imageName1;
                        @move_uploaded_file($tempName1,$pathComplete1);
                        $insertFarmerData['FarmerSignatureOrder']= $imageName1;
		}
			  
                        //echo '<pre>';
                        //print_r($insertFarmerData);
        // exit();
                        ...
                        $farmerMaster->updateOrderId($update, $rawOrderId);

                        /*$paymentrecord = array();
                        $paymentrecord['order_id'] = $finalOrderId;
                        $paymentrecord['received_amount'] = $payment_recevied;
...

		{
			...
			echo json_encode(array("error_code"=>'0', 'response_string'=>'Seed Order succuessfully done.','orderId'=>$finalOrderId));
	...
	*/
	...
	function getFarmerListAction(){
		...
		try	{
            ...
            $allChemicalOrderData = $farmerMaster->getChemicalOrderList($LoginID);
            $getPartPaymentListBySeedOrder = $farmerMaster->getPartPaymentListBySeedOrder($LoginID);
            $getPartPaymentListByChemicalOrder = $farmerMaster->getPartPaymentListByChemicalOrder($LoginID);
		...
		if($succes == TRUE )
		{
			echo json_encode(array("error_code"=>'0', 'response_string'=>'Farmer listed Successfully.','ImageUrl'=>'uploads/user_photograph/dummy_pic.png','Farmer_List'=>$allFarmerData,'Seed_Order_List'=>$allSeedOrderData, 'Part_payment_detail_by_seed_order'=>$getPartPaymentListBySeedOrder, 'Part_payment_detail_by_chemical_order'=>$getPartPaymentListByChemicalOrder, 'All_chemical_order'=>$allChemicalOrderData));
			exit;
...
if($alldata['Last_location_service_hit_time']!=date("Y-m-d H:i:s")){
...
/********************************************End of FE visit report Api*************************************************************/
	...
	function getHarvestListAction(){
		...
		$secretkey = $this->secretkey;
		$db = $this->db;
                $params = array('appkey'=>'teamlogimetrix@abhishek@naveen', 'LoginID'=>'SUF1010', 'device_id'=>'354101061265987','fe_code'=>'SUF1069','farmer_code'=>'SUF1069' ,'harvest_detail'=>'[{"net_price":"1","delivery":"1","harvest_name":"Moisture"}]');
		...
		try{
                    ...
                    foreach($harvest_data as $k){
                        ...
                        $db->insert('logi_hervestlist', $harvest_data_array);                            
                        $k++;
                    }
                    $db->commit();
                    $succes = TRUE;	
