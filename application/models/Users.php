...
//         }
	...
	public function getuser_id($UserID){
		...
		$userquery = "SELECT * FROM logi_field_users where LoginID='".$UserID."'";  
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public function getContactAllInformation($name, $mobile, $fuser,$address, $polling_both,$under){
		$db =  Zend_Db_Table::getDefaultAdapter();

		if($name){
			$cond .="and ci.name like '%".$name."%'";
		}

		if($mobile){
			$cond .="and ci.mobile like '%".$mobile."%'";
		}


		if($fuser){
			$cond .="and ci.user_id = '".$fuser."'";
		}

		if($address){
			$cond .="and ci.address = '".$address."'";
		}

		if($polling_both){
			$cond .="and ci.polling_both = '".$polling_both."'";
		}

		if($under){
			$cond .="and lu.id = '".$under."'";
		}
		$query = "select ci.*, ci.under as parent_user,lu.identity,lu.id from contact_info as ci left join logi_field_users as lu on(lu.identity = ci.under) where 1 and ci.project ='' $cond  order by ci.id desc";
		return $result = $db->fetchAll($query);
	}



	public function getUnderData($identity){
		$db =  Zend_Db_Table::getDefaultAdapter();
		$query = "select * from contact_info where under ='".$identity."'"; 
		return $result = $db->fetchAll($query);

	}

	public function getPlaceUnderData($identity){
		$db =  Zend_Db_Table::getDefaultAdapter();
		$query = "select * from place_info where under ='".$identity."'"; 
		return $result = $db->fetchAll($query);
	...
	public function getAllPlacedInformation($name, $mobile, $fuser,$under){
		$db =  Zend_Db_Table::getDefaultAdapter();
		...
		$query = "select pi.* , pi.under as parent_user, lu.identity,lu.id as identity_id from place_info as pi left join logi_field_users as lu on(lu.identity = pi.under) where 1 and pi.project =''  $cond  order by pi.id desc";
	...
	public function getUnderUserInfo(){
		$db =  Zend_Db_Table::getDefaultAdapter();
		$query = "select ci.under ,lu.identity, lu.id from contact_info as ci left join logi_field_users as lu on (lu.identity = ci.under) where ci.under !=''";
		return $result = $db->fetchAll($query);
	}
	...
	public function getPlaceUnderUserInfo(){
		$db =  Zend_Db_Table::getDefaultAdapter();
		$query = "select ci.under ,lu.identity, lu.id from place_info as ci left join logi_field_users as lu on (lu.identity = ci.under) where ci.under !=''";
		return $result = $db->fetchAll($query);
	}

	public function getTotalCountCapturedInfoDetail(){
		$db =  Zend_Db_Table::getDefaultAdapter();
		$query = "select count(id) as info_count from contact_info";
		return $result = $db->fetchRow($query);
	}
	...
	public function getTotalCountCapturedPlaceDetail(){
		$db =  Zend_Db_Table::getDefaultAdapter();
		$query = "select count(id) as place_count from place_info";
		return $result = $db->fetchRow($query);
	}
	...
	public function getContactAllInformationCount( $fuser){
		...
		$query = " select count(id) as count_info from contact_info where 1 $cond";     
		return $result = $db->fetchRow($query);
	}
	...
	public function getAllPlacedInformationCount($fuser){
		...
		$query = "select count(id) as count_place_info from place_info where 1  $cond"; 
		return $result = $db->fetchRow($query);
	}

	public function GetAllFeUserList($floginid, $login_status, $date){
		$db =  Zend_Db_Table::getDefaultAdapter();

		if($floginid){
			$cond .=" and LoginID = '".$floginid."'";
		}

		if($login_status && $login_status =='yes' && $floginid ==''){

			$getLoginId = array();
			$get_feLoginId = "select mob_user_staff_code from logi_user_login_detail where date(login_time) =  '".$date."' $cond and WebLoginID !='admin' group by mob_user_staff_code";
			$result_Login = $db->fetchAll($get_feLoginId);
			foreach($result_Login as $loginDeatil){
				array_push($getLoginId, "'".$loginDeatil['mob_user_staff_code']."'");
			}
			$totalRecord = implode(",", $getLoginId);
			$cond .="and LoginID IN ($totalRecord)";      	
		}


		if($login_status && $login_status =='no' && $floginid ==''){

			$getLoginId = array();
			$get_feLogiId = "select mob_user_staff_code from logi_user_login_detail where date(login_time) =  '".$date."' $cond and WebLoginID !='admin' group by mob_user_staff_code";
			$result_Login = $db->fetchAll($get_feLogiId);
			foreach($result_Login as $loginDeatil){
				array_push($getLoginId, "'".$loginDeatil['mob_user_staff_code']."'");
			}
			$totalRecord = implode(",", $getLoginId);
			$cond .="and LoginID NOT IN ($totalRecord)";      	
		}

		$query = "SELECT StaffName,LoginID,Email,MobileNo FROM logi_field_users 
		WHERE StaffStatus='AC' $cond and role='field_user' and project = '' order by id DESC"; 
		return $result = $db->fetchAll($query);
	}


	public function getUserLoginDetailByLoginId($floginid, $date_check){
		$db =  Zend_Db_Table::getDefaultAdapter();

		if($floginid){
			$cond .=" and mob_user_staff_code = '".$floginid."'";

		}

		$query = "select *, date(login_time) as Login_date from logi_user_login_detail where date(login_time) =  '".$date_check."' $cond and WebLoginID !='admin' group by mob_user_staff_code";

		return $result = $db->fetchAll($query);
	}


	public function getContactAllInformationByPollingBoth($polling_both){
		$db =  Zend_Db_Table::getDefaultAdapter();

		if($polling_both){
			$cond .="and polling_both like '%".$polling_both."%'";
		}
	...
	public function deleteUser($userId){
		...
		$delContactInfo = "delete from contact_info where user_id='".$userId."'";
		$db->query($delContactInfo);

			//delete user contact info
		$delPlaceInfo = "delete from place_info where user_id='".$userId."'";
		$db->query($delPlaceInfo);

		$delUser = "delete from logi_field_users where LoginID='".$userId."'";
		$db->query($delUser);
